...
ROOT CAUSE FIXES:
- Uses ImprovedSNN (correct architecture matching snn_model.py)
- Applies StandardScaler normalization BEFORE spike encoding
- Converts features to proper spike trains (shape: 1 x features x time_steps)
- Loads scaler from disk (saved during training)
- Uses the optimal_threshold saved in the checkpoint
- FIXED: Proper feature extraction for 5 features
- FIXED: NaN handling in normalization
- FIXED: PyTorch 2.6+ compatibility with weights_only parameter
- ADDED: Flask-SocketIO for real-time WebSocket data streaming to web dashboard
"""
import serial
import torch
...
# ─────────────────────────────────────────────
# FLASK + SOCKET.IO SETUP (NEW: WebSocket Backend)
# ─────────────────────────────────────────────
...
app.config['SECRET_KEY'] = 'snn-monitor-secret-2026'
...
def handle_connect():
    ...
    connected_clients += 1
    print(f"🔗 Client connected ({connected_clients} total)")
    emit('system_status', {
        'status': 'connected',
        'message': 'SNN Monitor Backend Ready',
        'timestamp': time.time()
    })
...
def handle_disconnect():
    ...
    print(f"🔌 Client disconnected ({connected_clients} remaining)")

@socketio.on('client_ready')
def handle_client_ready(data):
    """Acknowledge client is ready to receive data"""
    print(f"✅ Client ready: {data.get('user_agent', 'unknown')}")
    emit('backend_ready', {
...
def emit_sensor_data(sample_data):
    ...
    # Rate limiting: only emit if enough time has passed
    ...
    try:
        ...
        # Don't crash the main loop on emit errors
...
print("\n" + "=" * 60)
print("FIXED SNN MONITOR + WEBSOCKET SERVER")
print("=" * 60)
...
# Start Flask in daemon thread (won't block main serial loop)
...
print(f"   WebSocket : Active on port 5000")
    ...
    while True:
        ...
        try:
            ...
            if debug_samples < 5:
                ...
                print(f"   [DEBUG] Norm: {norm_features.tolist()}")
                debug_samples += 1
            elif count % 500 == 0:
                print(f"\n[STATS] Sample #{count}: Anomaly rate: {anomaly_count/count*100:.1f}%")
            
            # 6. 🚀 EMIT TO WEBSOCKET (NEW: Send to frontend)
            sample_data = {
                'count': count,
                'ax': raw_features[0],
                'ay': raw_features[1],
